...
inst_debug_func callback_inst_debug = NULL;
...
{
    SR[rt] = REGVAL(*(CR[rd %= NUMBER_OF_CP0_REGISTERS]));
    SR[zero] = 0x00000000;
    if (rd == 0x7) {
        if (CFG_MEND_SEMAPHORE_LOCK == 0)
            return;
        GET_RCP_REG(SP_SEMAPHORE_REG) = REGVAL(0x00000001);
        GET_RCP_REG(SP_STATUS_REG) |= REGVAL(SP_STATUS_HALT); /* temporary hack */
        return;
    }
#ifdef WAIT_FOR_CPU_HOST
    if (rd == 0x4) {
        MFC0_count[rt] += 1;
        GET_RCP_REG(SP_STATUS_REG) |= REGVAL(MFC0_count[rt] >= MF_SP_STATUS_TIMEOUT);
    }
#endif
    return;
}
...
static void MT_SP_STATUS(unsigned int rt)
{
    ...
    *SP_STATUS_REG &= REGVAL(~(!!(SR[rt] & 0x00000001) <<  0));
    *SP_STATUS_REG &= REGVAL(~(!!(SR[rt] & 0x00000004) <<  1));
 /* DMA_BUSY, DMA_FULL, IO_FULL:  No feature exists to clear these. */
    *SP_STATUS_REG &= REGVAL(~(!!(SR[rt] & 0x00000020) <<  5));
    *SP_STATUS_REG &= REGVAL(~(!!(SR[rt] & 0x00000080) <<  6));
    *SP_STATUS_REG &= REGVAL(~(!!(SR[rt] & 0x00000200) <<  7));
    *SP_STATUS_REG &= REGVAL(~(!!(SR[rt] & 0x00000800) <<  8));
    *SP_STATUS_REG &= REGVAL(~(!!(SR[rt] & 0x00002000) <<  9));
    *SP_STATUS_REG &= REGVAL(~(!!(SR[rt] & 0x00008000) << 10));
    *SP_STATUS_REG &= REGVAL(~(!!(SR[rt] & 0x00020000) << 11));
    *SP_STATUS_REG &= REGVAL(~(!!(SR[rt] & 0x00080000) << 12));
    *SP_STATUS_REG &= REGVAL(~(!!(SR[rt] & 0x00200000) << 13));
    *SP_STATUS_REG &= REGVAL(~(!!(SR[rt] & 0x00800000) << 14));

    *SP_STATUS_REG |=  REGVAL(!!(SR[rt] & 0x00000002) <<  0);
 /* No feature exists to set BROKE:  (!!1 << 1) */
 /* DMA_BUSY, DMA_FULL, IO_FULL:  No feature exists to set these. */
    *SP_STATUS_REG |=  REGVAL(!!(SR[rt] & 0x00000040) <<  5);
    *SP_STATUS_REG |=  REGVAL(!!(SR[rt] & 0x00000100) <<  6);
    *SP_STATUS_REG |=  REGVAL(!!(SR[rt] & 0x00000400) <<  7); /* yield request? */
    *SP_STATUS_REG |=  REGVAL(!!(SR[rt] & 0x00001000) <<  8); /* yielded? */
    *SP_STATUS_REG |=  REGVAL(!!(SR[rt] & 0x00004000) <<  9); /* task done? */
    *SP_STATUS_REG |=  REGVAL(!!(SR[rt] & 0x00010000) << 10);
    *SP_STATUS_REG |=  REGVAL(!!(SR[rt] & 0x00040000) << 11);
    *SP_STATUS_REG |=  REGVAL(!!(SR[rt] & 0x00100000) << 12);
    *SP_STATUS_REG |=  REGVAL(!!(SR[rt] & 0x00400000) << 13);
    *SP_STATUS_REG |=  REGVAL(!!(SR[rt] & 0x01000000) << 14);

    MI_INTR_REG = GET_RSP_INFO(MI_INTR_REG);
    *MI_INTR_REG   &= REGVAL(~((SR[rt] & 0x00000008) >> 3)); /* SP_CLR_INTR */
    *MI_INTR_REG   |=   REGVAL((SR[rt] & 0x00000010) >> 4); /* SP_SET_INTR */
    *SP_STATUS_REG |=   REGVAL((SR[rt] & 0x00000010) >> 4); /* int set halt */
    return;
}
static void MT_SP_RESERVED(unsigned int rt)
{
    ...
    GET_RCP_REG(SP_SEMAPHORE_REG) = REGVAL(source);
    return;
}
...
static void MT_CMD_END(unsigned int rt)
{
    ...
    GET_RCP_REG(DPC_END_REG) = REGVAL(SR[rt] & 0xFFFFFFF8ul);
    GBI_phase();
    return;
}
static void MT_CMD_STATUS(unsigned int rt)
{
    ...
    *DPC_STATUS_REG &= REGVAL(~(!!(SR[rt] & 0x00000001) << 0));
    *DPC_STATUS_REG |    REGVAL(!!(SR[rt] & 0x00000002) << 0);
    *DPC_STATUS_REG &= REGVAL(~(!!(SR[rt] & 0x00000004) << 1));
    *DPC_STATUS_REG |=   REGVAL(!!(SR[rt] & 0x00000008) << 1);
    *DPC_STATUS_REG &= REGVAL(~(!!(SR[rt] & 0x00000010) << 2));
    *DPC_STATUS_REG |=   REGVAL(!!(SR[rt] & 0x00000020) << 2);
/* Some NUS-CIC-6105 SP tasks try to clear some DPC cycle timers. */
    GET_RCP_REG(DPC_TMEM_REG)     &= REGVAL(!(SR[rt] & 0x00000040) ? ~0u : 0u);
 /* GET_RCP_REG(DPC_PIPEBUSY_REG) &= !(SR[rt] & 0x00000080) ? ~0u : 0u; */
 /* GET_RCP_REG(DPC_BUFBUSY_REG)  &= !(SR[rt] & 0x00000100) ? ~0u : 0u; */
    GET_RCP_REG(DPC_CLOCK_REG)    &= REGVAL(!(SR[rt] & 0x00000200) ? ~0u : 0u);
    return;
}
static void MT_CMD_CLOCK(unsigned int rt)
{
    message("MTC0\nCMD_CLOCK"); /* read-only?? */
    GET_RCP_REG(DPC_CLOCK_REG) = REGVAL(SR[rt]);
...
void SP_DMA_READ(void)
{
    ...
    length = (REGVAL(GET_RCP_REG(SP_RD_LEN_REG)) & 0x00000FFFul) >>  0;
    count  = (REGVAL(GET_RCP_REG(SP_RD_LEN_REG)) & 0x000FF000ul) >> 12;
    skip   = (REGVAL(GET_RCP_REG(SP_RD_LEN_REG)) & 0xFFF00000ul) >> 20;
#ifdef _DEBUG
    ...
    do {
        ...
        --count;
        do {
            offC = (count*length + REGVAL(*CR[0x0]) + i) & 0x00001FF8ul;
            offD = (count*skip +   REGVAL(*CR[0x1]) + i) & 0x00FFFFF8ul;
            i += 0x008;
    ...
    if ((REGVAL(*CR[0x0]) ^ offC) & 0x1000)
        message("DMA over the DMEM-to-IMEM gap.");
    GET_RCP_REG(SP_DMA_BUSY_REG)  =  0x00000000;
    GET_RCP_REG(SP_STATUS_REG)   &= REGVAL(~SP_STATUS_DMA_BUSY);
    return;
}
void SP_DMA_WRITE(void)
{
    ...
    length = (REGVAL(GET_RCP_REG(SP_WR_LEN_REG)) & 0x00000FFFul) >>  0;
    count  = (REGVAL(GET_RCP_REG(SP_WR_LEN_REG)) & 0x000FF000ul) >> 12;
    skip   = (REGVAL(GET_RCP_REG(SP_WR_LEN_REG)) & 0xFFF00000ul) >> 20;
...
PROFILE_MODE void LH(u32 inst)
{
    ...
    addr = SR[base] + offset;
    SR[rt] = 0x00000000
      | DMEM[(addr + 0) & 0x00000FFFul] <<  8
      | DMEM[(addr + 1) & 0x00000FFFul] <<  0
    ;
    SR[rt] = (s16)SR[rt];
    SR[zero] = 0x00000000;
}
PROFILE_MODE void LW(u32 inst)
{
    ...
    addr = SR[base] + offset;
    SR_B(rt, 0) = DMEM[(addr + 0) & 0x00000FFFul];
    SR_B(rt, 1) = DMEM[(addr + 1) & 0x00000FFFul];
    SR_B(rt, 2) = DMEM[(addr + 2) & 0x00000FFFul];
    SR_B(rt, 3) = DMEM[(addr + 3) & 0x00000FFFul];
    SR[zero] = 0x00000000;
}
...
PROFILE_MODE void SB(u32 inst)
{
    ...
    addr = SR[base] + offset;
    DMEM[(addr) & 0x00000FFFul] = (u8)(SR[rt] & 0xFFu);
}
PROFILE_MODE void SH(u32 inst)
{
    ...
    addr = SR[base] + offset;
    DMEM[(addr + 0) & 0x00000FFFul] = SR_B(rt, 2);
    DMEM[(addr + 1) & 0x00000FFFul] = SR_B(rt, 3);
}
PROFILE_MODE void SW(u32 inst)
{
    ...
    addr = SR[base] + offset;
    DMEM[(addr + 0) & 0x00000FFFul] = SR_B(rt, 0);
    DMEM[(addr + 1) & 0x00000FFFul] = SR_B(rt, 1);
    DMEM[(addr + 2) & 0x00000FFFul] = SR_B(rt, 2);
    DMEM[(addr + 3) & 0x00000FFFul] = SR_B(rt, 3);
}
...
void LSV(unsigned vt, unsigned element, signed offset, unsigned base)
{
    ...
    VR_S(vt, e) = MEM16(*(pi16)(DMEM + addr - correction));
    return;
}
void LLV(unsigned vt, unsigned element, signed offset, unsigned base)
{
    ...
    VR_S(vt, e+0x0) = MEM16(*(pi16)(DMEM + addr - correction));
    addr = (addr + 0x00000002) & 0x00000FFF; /* F3DLX 1.23:  addr%4 is 0x002. */
    VR_S(vt, e+0x2) = MEM16(*(pi16)(DMEM + addr + correction));
    return;
}
void LDV(unsigned vt, unsigned element, signed offset, unsigned base)
{
    ...
    switch (addr & 07) {
    case 00:
        VR_S(vt, e+0x0) = MEM16(*(pi16)(DMEM + addr + HES(0x000)));
        VR_S(vt, e+0x2) = MEM16(*(pi16)(DMEM + addr + HES(0x002)));
        VR_S(vt, e+0x4) = MEM16(*(pi16)(DMEM + addr + HES(0x004)));
        VR_S(vt, e+0x6) = MEM16(*(pi16)(DMEM + addr + HES(0x006)));
        break;
    case 01: /* standard ABI ucodes (unlike e.g. MusyX w/ even addresses) */
        VR_S(vt, e+0x0) = MEM16(*(pi16)(DMEM + addr + 0x000));
        VR_A(vt, e+0x2) = DMEM[addr + 0x002 - BES(0x000)];
        VR_U(vt, e+0x3) = DMEM[addr + 0x003 + BES(0x000)];
        VR_S(vt, e+0x4) = MEM16(*(pi16)(DMEM + addr + 0x004));
        ...
        break;
    case 02:
        VR_S(vt, e+0x0) = MEM16(*(pi16)(DMEM + addr + 0x000 - HES(0x000)));
        VR_S(vt, e+0x2) = MEM16(*(pi16)(DMEM + addr + 0x002 + HES(0x000)));
        VR_S(vt, e+0x4) = MEM16(*(pi16)(DMEM + addr + 0x004 - HES(0x000)));
        addr += 0x006 + HES(00);
        addr &= 0x00000FFF;
        VR_S(vt, e+0x6) = MEM16(*(pi16)(DMEM + addr));
        break;
    case 03: /* standard ABI ucodes (unlike e.g. MusyX w/ even addresses) */
        ...
        VR_S(vt, e+0x2) = MEM16(*(pi16)(DMEM + addr + 0x002));
        VR_A(vt, e+0x4) = DMEM[addr + 0x004 - BES(0x000)];
        addr += 0x005 + BES(00);
        addr &= 0x00000FFF;
        VR_U(vt, e+0x5) = DMEM[addr];
        VR_S(vt, e+0x6) = MEM16(*(pi16)(DMEM + addr + 0x001 - BES(0x000)));
        break;
    case 04:
        VR_S(vt, e+0x0) = MEM16(*(pi16)(DMEM + addr + HES(0x000)));
        VR_S(vt, e+0x2) = MEM16(*(pi16)(DMEM + addr + HES(0x002)));
        addr += 0x004 + WES(00);
        addr &= 0x00000FFF;
        VR_S(vt, e+0x4) = MEM16(*(pi16)(DMEM + addr + HES(0x000)));
        VR_S(vt, e+0x6) = MEM16(*(pi16)(DMEM + addr + HES(0x002)));
        break;
    case 05: /* standard ABI ucodes (unlike e.g. MusyX w/ even addresses) */
        ...
        VR_S(vt, e+0x4) = MEM16(*(pi16)(DMEM + addr + 0x001));
    ...
    case 06:
        ...
        addr += 0x002;
        addr &= 0x00000FFF;
        VR_S(vt, e+0x2) = MEM16(*(pi16)(DMEM + addr + HES(0x000)));
        VR_S(vt, e+0x4) = MEM16(*(pi16)(DMEM + addr + HES(0x002)));
        VR_S(vt, e+0x6) = MEM16(*(pi16)(DMEM + addr + HES(0x004)));
        break;
    case 07: /* standard ABI ucodes (unlike e.g. MusyX w/ even addresses) */
        ...
        VR_S(vt, e+0x2) = MEM16(*(pi16)(DMEM + addr + 0x001));
        VR_A(vt, e+0x4) = DMEM[addr + BES(0x003)];
        VR_U(vt, e+0x5) = DMEM[addr + BES(0x004)];
        VR_S(vt, e+0x6) = MEM16(*(pi16)(DMEM + addr + 0x005));
        break;
    }
    return;
}
...
void SLV(unsigned vt, unsigned element, signed offset, unsigned base)
{
    ...
    correction = HES(0x000)*(addr%0x004 - 1);
    *(pi16)(DMEM + addr - correction) = MEM16(VR_S(vt, e+0x0));
    addr = (addr + 0x00000002) & 0x00000FFF; /* F3DLX 0.95:  "Mario Kart 64" */
    *(pi16)(DMEM + addr + correction) = MEM16(VR_S(vt, e+0x2));
    return;
}
void SDV(unsigned vt, unsigned element, signed offset, unsigned base)
{
    ...
    switch (addr & 07) {
    case 00:
        *(pi16)(DMEM + addr + HES(0x000)) = MEM16(VR_S(vt, e+0x0));
        *(pi16)(DMEM + addr + HES(0x002)) = MEM16(VR_S(vt, e+0x2));
        *(pi16)(DMEM + addr + HES(0x004)) = MEM16(VR_S(vt, e+0x4));
        *(pi16)(DMEM + addr + HES(0x006)) = MEM16(VR_S(vt, e+0x6));
        break;
    case 01: /* "Tetrisphere" audio ucode */
        *(pi16)(DMEM + addr + 0x000) = MEM16(VR_S(vt, e+0x0));
        DMEM[addr + 0x002 - BES(0x000)] = VR_A(vt, e+0x2);
        DMEM[addr + 0x003 + BES(0x000)] = VR_U(vt, e+0x3);
        *(pi16)(DMEM + addr + 0x004) = MEM16(VR_S(vt, e+0x4));
        ...
        break;
    case 02:
        *(pi16)(DMEM + addr + 0x000 - HES(0x000)) = MEM16(VR_S(vt, e+0x0));
        *(pi16)(DMEM + addr + 0x002 + HES(0x000)) = MEM16(VR_S(vt, e+0x2));
        *(pi16)(DMEM + addr + 0x004 - HES(0x000)) = MEM16(VR_S(vt, e+0x4));
        addr += 0x006 + HES(0x000);
        addr &= 0x00000FFF;
        *(pi16)(DMEM + addr) = MEM16(VR_S(vt, e+0x6));
        break;
    case 03: /* "Tetrisphere" audio ucode */
        DMEM[addr + 0x000 - BES(0x000)] = VR_A(vt, e+0x0);
        DMEM[addr + 0x001 + BES(0x000)] = VR_U(vt, e+0x1);
        *(pi16)(DMEM + addr + 0x002) = MEM16(VR_S(vt, e+0x2));
        DMEM[addr + 0x004 - BES(0x000)] = VR_A(vt, e+0x4);
        addr += 0x005 + BES(0x000);
        addr &= 0x00000FFF;
        DMEM[addr] = VR_U(vt, e+0x5);
        *(pi16)(DMEM + addr + 0x001 - BES(0x000)) = MEM16(VR_S(vt, 0x6));
        break;
    case 04:
        *(pi16)(DMEM + addr + HES(0x000)) = MEM16(VR_S(vt, e+0x0));
        *(pi16)(DMEM + addr + HES(0x002)) = MEM16(VR_S(vt, e+0x2));
        addr = (addr + 0x004) & 0x00000FFF;
        *(pi16)(DMEM + addr + HES(0x000)) = MEM16(VR_S(vt, e+0x4));
        *(pi16)(DMEM + addr + HES(0x002)) = MEM16(VR_S(vt, e+0x6));
        break;
    case 05: /* "Tetrisphere" audio ucode */
        ...
        *(pi16)(DMEM + addr + 0x001) = MEM16(VR_S(vt, e+0x4));
    ...
    case 06:
        ...
        *(pi16)(DMEM + addr + HES(0x000)) = MEM16(VR_S(vt, e+0x2));
        *(pi16)(DMEM + addr + HES(0x002)) = MEM16(VR_S(vt, e+0x4));
        *(pi16)(DMEM + addr + HES(0x004)) = MEM16(VR_S(vt, e+0x6));
        break;
    case 07: /* "Tetrisphere" audio ucode */
        ...
        *(pi16)(DMEM + addr + 0x001) = MEM16(VR_S(vt, e+0x2));
        DMEM[addr + BES(0x003)] = VR_A(vt, e+0x4);
        DMEM[addr + BES(0x004)] = VR_U(vt, e+0x5);
        *(pi16)(DMEM + addr + 0x005) = MEM16(VR_S(vt, e+0x6));
        break;
    }
    return;
}
...
void LPV(unsigned vt, unsigned element, signed offset, unsigned base)
{
    ...
    switch (b) {
    case 00:
        VR[vt][07] = MEM16(DMEM[addr + BES(0x007)] << 8);
        VR[vt][06] = MEM16(DMEM[addr + BES(0x006)] << 8);
        VR[vt][05] = MEM16(DMEM[addr + BES(0x005)] << 8);
        VR[vt][04] = MEM16(DMEM[addr + BES(0x004)] << 8);
        VR[vt][03] = MEM16(DMEM[addr + BES(0x003)] << 8);
        VR[vt][02] = MEM16(DMEM[addr + BES(0x002)] << 8);
        VR[vt][01] = MEM16(DMEM[addr + BES(0x001)] << 8);
        VR[vt][00] = MEM16(DMEM[addr + BES(0x000)] << 8);
        break;
    case 01: /* F3DZEX 2.08J "Doubutsu no Mori" (Animal Forest) CPU CFB */
        VR[vt][00] = MEM16(DMEM[addr + BES(0x001)] << 8);
        VR[vt][01] = DMEM[addr + BES(0x002)] << 8;
        VR[vt][02] = MEM16( DMEM[addr + BES(0x003)] << 8);
        VR[vt][03] = MEM16( DMEM[addr + BES(0x004)] << 8);
        VR[vt][04] = MEM16( DMEM[addr + BES(0x005)] << 8);
        VR[vt][05] = MEM16( DMEM[addr + BES(0x006)] << 8);
        VR[vt][06] = MEM16( DMEM[addr + BES(0x007)] << 8);
        addr += BES(0x008);
        addr &= 0x00000FFF;
        VR[vt][07] = DMEM[addr] << 8;
        break;
    case 02: /* F3DZEX 2.08J "Doubutsu no Mori" (Animal Forest) CPU CFB */
        VR[vt][00] = MEM16( DMEM[addr + BES(0x002)] << 8);
        VR[vt][01] = MEM16( DMEM[addr + BES(0x003)] << 8);
        VR[vt][02] = MEM16( DMEM[addr + BES(0x004)] << 8);
        VR[vt][03] = MEM16( DMEM[addr + BES(0x005)] << 8);
        VR[vt][04] = MEM16( DMEM[addr + BES(0x006)] << 8);
        VR[vt][05] = MEM16( DMEM[addr + BES(0x007)] << 8);
        addr += 0x008;
        addr &= 0x00000FFF;
        VR[vt][06] = MEM16( DMEM[addr + BES(0x000)] << 8);
        VR[vt][07] = MEM16( DMEM[addr + BES(0x001)] << 8);
        break;
    case 03: /* F3DZEX 2.08J "Doubutsu no Mori" (Animal Forest) CPU CFB */
        VR[vt][00] = MEM16( DMEM[addr + BES(0x003)] << 8);
        VR[vt][01] = MEM16( DMEM[addr + BES(0x004)] << 8);
        VR[vt][02] = MEM16( DMEM[addr + BES(0x005)] << 8);
        VR[vt][03] = MEM16( DMEM[addr + BES(0x006)] << 8);
        VR[vt][04] = MEM16( DMEM[addr + BES(0x007)] << 8);
        addr += 0x008;
        addr &= 0x00000FFF;
        VR[vt][05] = MEM16( DMEM[addr + BES(0x000)] << 8);
        VR[vt][06] = MEM16( DMEM[addr + BES(0x001)] << 8);
        VR[vt][07] = MEM16( DMEM[addr + BES(0x002)] << 8);
        break;
    case 04: /* "Resident Evil 2" in-game 3-D, F3DLX 2.08--"WWF No Mercy" */
        VR[vt][00] = MEM16( DMEM[addr + BES(0x004)] << 8);
        VR[vt][01] = MEM16( DMEM[addr + BES(0x005)] << 8);
        VR[vt][02] = MEM16( DMEM[addr + BES(0x006)] << 8);
        VR[vt][03] = MEM16( DMEM[addr + BES(0x007)] << 8);
        addr += 0x008;
        addr &= 0x00000FFF;
        VR[vt][04] = MEM16( DMEM[addr + BES(0x000)] << 8);
        VR[vt][05] = MEM16( DMEM[addr + BES(0x001)] << 8);
        VR[vt][06] = MEM16( DMEM[addr + BES(0x002)] << 8);
        VR[vt][07] = MEM16( DMEM[addr + BES(0x003)] << 8);
        break;
    case 05: /* F3DZEX 2.08J "Doubutsu no Mori" (Animal Forest) CPU CFB */
        VR[vt][00] = MEM16( DMEM[addr + BES(0x005)] << 8);
        VR[vt][01] = MEM16( DMEM[addr + BES(0x006)] << 8);
        VR[vt][02] = MEM16( DMEM[addr + BES(0x007)] << 8);
        addr += 0x008;
        addr &= 0x00000FFF;
        VR[vt][03] = MEM16( DMEM[addr + BES(0x000)] << 8);
        VR[vt][04] = MEM16( DMEM[addr + BES(0x001)] << 8);
        VR[vt][05] = MEM16( DMEM[addr + BES(0x002)] << 8);
        VR[vt][06] = MEM16( DMEM[addr + BES(0x003)] << 8);
        VR[vt][07] = MEM16( DMEM[addr + BES(0x004)] << 8);
        break;
    case 06: /* F3DZEX 2.08J "Doubutsu no Mori" (Animal Forest) CPU CFB */
        VR[vt][00] = MEM16( DMEM[addr + BES(0x006)] << 8);
        VR[vt][01] = MEM16( DMEM[addr + BES(0x007)] << 8);
        addr += 0x008;
        addr &= 0x00000FFF;
        VR[vt][02] = MEM16( DMEM[addr + BES(0x000)] << 8);
        VR[vt][03] = MEM16( DMEM[addr + BES(0x001)] << 8);
        VR[vt][04] = MEM16( DMEM[addr + BES(0x002)] << 8);
        VR[vt][05] = MEM16( DMEM[addr + BES(0x003)] << 8);
        VR[vt][06] = MEM16( DMEM[addr + BES(0x004)] << 8);
        VR[vt][07] = MEM16( DMEM[addr + BES(0x005)] << 8);
        break;
    case 07: /* F3DZEX 2.08J "Doubutsu no Mori" (Animal Forest) CPU CFB */
        VR[vt][00] = MEM16( DMEM[addr + BES(0x007)] << 8);
        addr += 0x008;
        addr &= 0x00000FFF;
        VR[vt][01] = MEM16( DMEM[addr + BES(0x000)] << 8);
        VR[vt][02] = MEM16( DMEM[addr + BES(0x001)] << 8);
        VR[vt][03] = MEM16( DMEM[addr + BES(0x002)] << 8);
        VR[vt][04] = MEM16( DMEM[addr + BES(0x003)] << 8);
        VR[vt][05] = MEM16( DMEM[addr + BES(0x004)] << 8);
        VR[vt][06] = MEM16( DMEM[addr + BES(0x005)] << 8);
        VR[vt][07] = MEM16( DMEM[addr + BES(0x006)] << 8);
        break;
    }
    return;
}
void LUV(unsigned vt, unsigned element, signed offset, unsigned base)
{
    ...
    switch (b) {
    case 00:
        VR[vt][07] = MEM16( DMEM[addr + BES(0x007)] << 7);
        VR[vt][06] = MEM16( DMEM[addr + BES(0x006)] << 7);
        VR[vt][05] = MEM16( DMEM[addr + BES(0x005)] << 7);
        VR[vt][04] = MEM16( DMEM[addr + BES(0x004)] << 7);
        VR[vt][03] = MEM16( DMEM[addr + BES(0x003)] << 7);
        VR[vt][02] = MEM16( DMEM[addr + BES(0x002)] << 7);
        VR[vt][01] = MEM16( DMEM[addr + BES(0x001)] << 7);
        VR[vt][00] = MEM16( DMEM[addr + BES(0x000)] << 7);
        break;
    case 01: /* PKMN Puzzle League HVQM decoder */
        VR[vt][00] = MEM16( DMEM[addr + BES(0x001)] << 7);
        VR[vt][01] = MEM16( DMEM[addr + BES(0x002)] << 7);
        VR[vt][02] = MEM16( DMEM[addr + BES(0x003)] << 7);
        VR[vt][03] = MEM16( DMEM[addr + BES(0x004)] << 7);
        VR[vt][04] = MEM16( DMEM[addr + BES(0x005)] << 7);
        VR[vt][05] = MEM16( DMEM[addr + BES(0x006)] << 7);
        VR[vt][06] = MEM16( DMEM[addr + BES(0x007)] << 7);
        addr += BES(0x008);
        addr &= 0x00000FFF;
        VR[vt][07] = DMEM[addr] << 7;
        break;
    case 02: /* PKMN Puzzle League HVQM decoder */
        VR[vt][00] = MEM16( DMEM[addr + BES(0x002)] << 7);
        VR[vt][01] = MEM16( DMEM[addr + BES(0x003)] << 7);
        VR[vt][02] = MEM16( DMEM[addr + BES(0x004)] << 7);
        VR[vt][03] = MEM16( DMEM[addr + BES(0x005)] << 7);
        VR[vt][04] = MEM16( DMEM[addr + BES(0x006)] << 7);
        VR[vt][05] = MEM16( DMEM[addr + BES(0x007)] << 7);
        addr += 0x008;
        addr &= 0x00000FFF;
        VR[vt][06] = MEM16( DMEM[addr + BES(0x000)] << 7);
        VR[vt][07] = MEM16( DMEM[addr + BES(0x001)] << 7);
        break;
    case 03: /* PKMN Puzzle League HVQM decoder */
        VR[vt][00] = MEM16( DMEM[addr + BES(0x003)] << 7);
        VR[vt][01] = MEM16( DMEM[addr + BES(0x004)] << 7);
        VR[vt][02] = MEM16( DMEM[addr + BES(0x005)] << 7);
        VR[vt][03] = MEM16( DMEM[addr + BES(0x006)] << 7);
        VR[vt][04] = MEM16( DMEM[addr + BES(0x007)] << 7);
        addr += 0x008;
        addr &= 0x00000FFF;
        VR[vt][05] = MEM16( DMEM[addr + BES(0x000)] << 7);
        VR[vt][06] = MEM16( DMEM[addr + BES(0x001)] << 7);
        VR[vt][07] = MEM16( DMEM[addr + BES(0x002)] << 7);
        break;
    case 04: /* PKMN Puzzle League HVQM decoder */
        VR[vt][00] = MEM16( DMEM[addr + BES(0x004)] << 7);
        VR[vt][01] = MEM16( DMEM[addr + BES(0x005)] << 7);
        VR[vt][02] = MEM16( DMEM[addr + BES(0x006)] << 7);
        VR[vt][03] = MEM16( DMEM[addr + BES(0x007)] << 7);
        addr += 0x008;
        addr &= 0x00000FFF;
        VR[vt][04] = MEM16( DMEM[addr + BES(0x000)] << 7);
        VR[vt][05] = MEM16( DMEM[addr + BES(0x001)] << 7);
        VR[vt][06] = MEM16( DMEM[addr + BES(0x002)] << 7);
        VR[vt][07] = MEM16( DMEM[addr + BES(0x003)] << 7);
        break;
    case 05: /* PKMN Puzzle League HVQM decoder */
        VR[vt][00] = MEM16( DMEM[addr + BES(0x005)] << 7);
        VR[vt][01] = MEM16( DMEM[addr + BES(0x006)] << 7);
        VR[vt][02] = MEM16( DMEM[addr + BES(0x007)] << 7);
        addr += 0x008;
        addr &= 0x00000FFF;
        VR[vt][03] = MEM16( DMEM[addr + BES(0x000)] << 7);
        VR[vt][04] = MEM16( DMEM[addr + BES(0x001)] << 7);
        VR[vt][05] = MEM16( DMEM[addr + BES(0x002)] << 7);
        VR[vt][06] = MEM16( DMEM[addr + BES(0x003)] << 7);
        VR[vt][07] = MEM16( DMEM[addr + BES(0x004)] << 7);
        break;
    case 06: /* PKMN Puzzle League HVQM decoder */
        VR[vt][00] = MEM16( DMEM[addr + BES(0x006)] << 7);
        VR[vt][01] = MEM16( DMEM[addr + BES(0x007)] << 7);
        addr += 0x008;
        addr &= 0x00000FFF;
        VR[vt][02] = MEM16( DMEM[addr + BES(0x000)] << 7);
        VR[vt][03] = MEM16( DMEM[addr + BES(0x001)] << 7);
        VR[vt][04] = MEM16( DMEM[addr + BES(0x002)] << 7);
        VR[vt][05] = MEM16( DMEM[addr + BES(0x003)] << 7);
        VR[vt][06] = MEM16( DMEM[addr + BES(0x004)] << 7);
        VR[vt][07] = MEM16( DMEM[addr + BES(0x005)] << 7);
        break;
    case 07: /* PKMN Puzzle League HVQM decoder */
        VR[vt][00] = MEM16( DMEM[addr + BES(0x007)] << 7);
        addr += 0x008;
        addr &= 0x00000FFF;
        VR[vt][01] = MEM16( DMEM[addr + BES(0x000)] << 7);
        VR[vt][02] = MEM16( DMEM[addr + BES(0x001)] << 7);
        VR[vt][03] = MEM16( DMEM[addr + BES(0x002)] << 7);
        VR[vt][04] = MEM16( DMEM[addr + BES(0x003)] << 7);
        VR[vt][05] = MEM16( DMEM[addr + BES(0x004)] << 7);
        VR[vt][06] = MEM16( DMEM[addr + BES(0x005)] << 7);
        VR[vt][07] = MEM16( DMEM[addr + BES(0x006)] << 7);
        break;
    }
    return;
}
...
void LQV(unsigned vt, unsigned element, signed offset, unsigned base)
{
    ...
    case 0x0/2:
        ...
        VR_S(vt,e+0x4) = MEM16(*(pi16)(DMEM + addr + HES(0x004)));
        VR_S(vt,e+0x6) = MEM16(*(pi16)(DMEM + addr + HES(0x006)));
        VR_S(vt,e+0x8) = MEM16(*(pi16)(DMEM + addr + HES(0x008)));
        VR_S(vt,e+0xA) = MEM16(*(pi16)(DMEM + addr + HES(0x00A)));
        VR_S(vt,e+0xC) = MEM16(*(pi16)(DMEM + addr + HES(0x00C)));
        VR_S(vt,e+0xE) = MEM16(*(pi16)(DMEM + addr + HES(0x00E)));
        break;
    case 0x2/2:
        VR_S(vt,e+0x0) = MEM16(*(pi16)(DMEM + addr + HES(0x002)));
        VR_S(vt,e+0x2) = MEM16(*(pi16)(DMEM + addr + HES(0x004)));
        VR_S(vt,e+0x4) = MEM16(*(pi16)(DMEM + addr + HES(0x006)));
        VR_S(vt,e+0x6) = MEM16(*(pi16)(DMEM + addr + HES(0x008)));
        VR_S(vt,e+0x8) = MEM16(*(pi16)(DMEM + addr + HES(0x00A)));
        VR_S(vt,e+0xA) = MEM16(*(pi16)(DMEM + addr + HES(0x00C)));
        VR_S(vt,e+0xC) = MEM16(*(pi16)(DMEM + addr + HES(0x00E)));
        break;
    case 0x4/2:
        VR_S(vt,e+0x0) = MEM16(*(pi16)(DMEM + addr + HES(0x004)));
        VR_S(vt,e+0x2) = MEM16(*(pi16)(DMEM + addr + HES(0x006)));
        VR_S(vt,e+0x4) = MEM16(*(pi16)(DMEM + addr + HES(0x008)));
        VR_S(vt,e+0x6) = MEM16(*(pi16)(DMEM + addr + HES(0x00A)));
        VR_S(vt,e+0x8) = MEM16(*(pi16)(DMEM + addr + HES(0x00C)));
        VR_S(vt,e+0xA) = MEM16(*(pi16)(DMEM + addr + HES(0x00E)));
        break;
    case 0x6/2:
        VR_S(vt,e+0x0) = MEM16(*(pi16)(DMEM + addr + HES(0x006)));
        VR_S(vt,e+0x2) = MEM16(*(pi16)(DMEM + addr + HES(0x008)));
        VR_S(vt,e+0x4) = MEM16(*(pi16)(DMEM + addr + HES(0x00A)));
        VR_S(vt,e+0x6) = MEM16(*(pi16)(DMEM + addr + HES(0x00C)));
        VR_S(vt,e+0x8) = MEM16(*(pi16)(DMEM + addr + HES(0x00E)));
        break;
    case 0x8/2: /* "Resident Evil 2" cinematics and Boss Game Studios */
        VR_S(vt,e+0x0) = MEM16(*(pi16)(DMEM + addr + HES(0x008)));
        VR_S(vt,e+0x2) = MEM16(*(pi16)(DMEM + addr + HES(0x00A)));
        VR_S(vt,e+0x4) = MEM16(*(pi16)(DMEM + addr + HES(0x00C)));
        VR_S(vt,e+0x6) = MEM16(*(pi16)(DMEM + addr + HES(0x00E)));
        break;
    case 0xA/2: /* "Conker's Bad Fur Day" audio microcode by Rareware */
        VR_S(vt,e+0x0) = MEM16(*(pi16)(DMEM + addr + HES(0x00A)));
        VR_S(vt,e+0x2) = MEM16(*(pi16)(DMEM + addr + HES(0x00C)));
        VR_S(vt,e+0x4) = MEM16(*(pi16)(DMEM + addr + HES(0x00E)));
        break;
    case 0xC/2: /* "Conker's Bad Fur Day" audio microcode by Rareware */
        VR_S(vt,e+0x0) = MEM16(*(pi16)(DMEM + addr + HES(0x00C)));
        VR_S(vt,e+0x2) = MEM16(*(pi16)(DMEM + addr + HES(0x00E)));
        break;
    case 0xE/2: /* "Conker's Bad Fur Day" audio microcode by Rareware */
        VR_S(vt,e+0x0) = MEM16(*(pi16)(DMEM + addr + HES(0x00E)));
        break;
    }
    return;
}
void LRV(unsigned vt, unsigned element, signed offset, unsigned base)
{
    ...
    switch (b/2) {
    case 0xE/2:
        VR[vt][01] = MEM16(*(pi16)(DMEM + addr + HES(0x000)));
        VR[vt][02] = MEM16(*(pi16)(DMEM + addr + HES(0x002)));
        VR[vt][03] = MEM16(*(pi16)(DMEM + addr + HES(0x004)));
        VR[vt][04] = MEM16(*(pi16)(DMEM + addr + HES(0x006)));
        VR[vt][05] = MEM16(*(pi16)(DMEM + addr + HES(0x008)));
        VR[vt][06] = MEM16(*(pi16)(DMEM + addr + HES(0x00A)));
        VR[vt][07] = MEM16(*(pi16)(DMEM + addr + HES(0x00C)));
        break;
    case 0xC/2:
        VR[vt][02] = MEM16(*(pi16)(DMEM + addr + HES(0x000)));
        VR[vt][03] = MEM16(*(pi16)(DMEM + addr + HES(0x002)));
        VR[vt][04] = MEM16(*(pi16)(DMEM + addr + HES(0x004)));
        VR[vt][05] = MEM16(*(pi16)(DMEM + addr + HES(0x006)));
        VR[vt][06] = MEM16(*(pi16)(DMEM + addr + HES(0x008)));
        VR[vt][07] = MEM16(*(pi16)(DMEM + addr + HES(0x00A)));
        break;
    case 0xA/2:
        VR[vt][03] = MEM16(*(pi16)(DMEM + addr + HES(0x000)));
        VR[vt][04] = MEM16(*(pi16)(DMEM + addr + HES(0x002)));
        VR[vt][05] = MEM16(*(pi16)(DMEM + addr + HES(0x004)));
        VR[vt][06] = MEM16(*(pi16)(DMEM + addr + HES(0x006)));
        VR[vt][07] = MEM16(*(pi16)(DMEM + addr + HES(0x008)));
        break;
    case 0x8/2:
        VR[vt][04] = MEM16(*(pi16)(DMEM + addr + HES(0x000)));
        VR[vt][05] = MEM16(*(pi16)(DMEM + addr + HES(0x002)));
        VR[vt][06] = MEM16(*(pi16)(DMEM + addr + HES(0x004)));
        VR[vt][07] = MEM16(*(pi16)(DMEM + addr + HES(0x006)));
        break;
    case 0x6/2:
        VR[vt][05] = MEM16(*(pi16)(DMEM + addr + HES(0x000)));
        VR[vt][06] = MEM16(*(pi16)(DMEM + addr + HES(0x002)));
        VR[vt][07] = MEM16(*(pi16)(DMEM + addr + HES(0x004)));
        break;
    case 0x4/2:
        VR[vt][06] = MEM16(*(pi16)(DMEM + addr + HES(0x000)));
        VR[vt][07] = MEM16(*(pi16)(DMEM + addr + HES(0x002)));
        break;
    case 0x2/2:
        VR[vt][07] = MEM16(*(pi16)(DMEM + addr + HES(0x000)));
        break;
    case 0x0/2:
        break;
    }
...
void SQV(unsigned vt, unsigned element, signed offset, unsigned base)
{
    ...
    switch (b) {
    case 00:
        *(pi16)(DMEM + addr + HES(0x000)) = MEM16(VR[vt][00]);
        *(pi16)(DMEM + addr + HES(0x002)) = MEM16(VR[vt][01]);
        *(pi16)(DMEM + addr + HES(0x004)) = MEM16(VR[vt][02]);
        *(pi16)(DMEM + addr + HES(0x006)) = MEM16(VR[vt][03]);
        *(pi16)(DMEM + addr + HES(0x008)) = MEM16(VR[vt][04]);
        *(pi16)(DMEM + addr + HES(0x00A)) = MEM16(VR[vt][05]);
        *(pi16)(DMEM + addr + HES(0x00C)) = MEM16(VR[vt][06]);
        *(pi16)(DMEM + addr + HES(0x00E)) = MEM16(VR[vt][07]);
        break;
    case 02:
        *(pi16)(DMEM + addr + HES(0x002)) = MEM16(VR[vt][00]);
        *(pi16)(DMEM + addr + HES(0x004)) = MEM16(VR[vt][01]);
        *(pi16)(DMEM + addr + HES(0x006)) = MEM16(VR[vt][02]);
        *(pi16)(DMEM + addr + HES(0x008)) = MEM16(VR[vt][03]);
        *(pi16)(DMEM + addr + HES(0x00A)) = MEM16(VR[vt][04]);
        *(pi16)(DMEM + addr + HES(0x00C)) = MEM16(VR[vt][05]);
        *(pi16)(DMEM + addr + HES(0x00E)) = MEM16(VR[vt][06]);
        break;
    case 04:
        *(pi16)(DMEM + addr + HES(0x004)) = MEM16(VR[vt][00]);
        *(pi16)(DMEM + addr + HES(0x006)) = MEM16(VR[vt][01]);
        *(pi16)(DMEM + addr + HES(0x008)) = MEM16(VR[vt][02]);
        *(pi16)(DMEM + addr + HES(0x00A)) = MEM16(VR[vt][03]);
        *(pi16)(DMEM + addr + HES(0x00C)) = MEM16(VR[vt][04]);
        *(pi16)(DMEM + addr + HES(0x00E)) = MEM16(VR[vt][05]);
        break;
    case 06:
        *(pi16)(DMEM + addr + HES(0x006)) = MEM16(VR[vt][00]);
        *(pi16)(DMEM + addr + HES(0x008)) = MEM16(VR[vt][01]);
        *(pi16)(DMEM + addr + HES(0x00A)) = MEM16(VR[vt][02]);
        *(pi16)(DMEM + addr + HES(0x00C)) = MEM16(VR[vt][03]);
        *(pi16)(DMEM + addr + HES(0x00E)) = MEM16(VR[vt][04]);
        break;
    default:
        message("SQV\nWeird addr.");
    }
...
void SRV(unsigned vt, unsigned element, signed offset, unsigned base)
{
    ...
    switch (b/2) {
    case 0xE/2:
        *(pi16)(DMEM + addr + HES(0x000)) = MEM16(VR[vt][01]);
        *(pi16)(DMEM + addr + HES(0x002)) = MEM16(VR[vt][02]);
        *(pi16)(DMEM + addr + HES(0x004)) = MEM16(VR[vt][03]);
        *(pi16)(DMEM + addr + HES(0x006)) = MEM16(VR[vt][04]);
        *(pi16)(DMEM + addr + HES(0x008)) = MEM16(VR[vt][05]);
        *(pi16)(DMEM + addr + HES(0x00A)) = MEM16(VR[vt][06]);
        *(pi16)(DMEM + addr + HES(0x00C)) = MEM16(VR[vt][07]);
        break;
    case 0xC/2:
        *(pi16)(DMEM + addr + HES(0x000)) = MEM16(VR[vt][02]);
        *(pi16)(DMEM + addr + HES(0x002)) = MEM16(VR[vt][03]);
        *(pi16)(DMEM + addr + HES(0x004)) = MEM16(VR[vt][04]);
        *(pi16)(DMEM + addr + HES(0x006)) = MEM16(VR[vt][05]);
        *(pi16)(DMEM + addr + HES(0x008)) = MEM16(VR[vt][06]);
        *(pi16)(DMEM + addr + HES(0x00A)) = MEM16(VR[vt][07]);
        break;
    case 0xA/2:
        *(pi16)(DMEM + addr + HES(0x000)) = MEM16(VR[vt][03]);
        *(pi16)(DMEM + addr + HES(0x002)) = MEM16(VR[vt][04]);
        *(pi16)(DMEM + addr + HES(0x004)) = MEM16(VR[vt][05]);
        *(pi16)(DMEM + addr + HES(0x006)) = MEM16(VR[vt][06]);
        *(pi16)(DMEM + addr + HES(0x008)) = MEM16(VR[vt][07]);
        break;
    case 0x8/2:
        *(pi16)(DMEM + addr + HES(0x000)) = MEM16(VR[vt][04]);
        *(pi16)(DMEM + addr + HES(0x002)) = MEM16(VR[vt][05]);
        *(pi16)(DMEM + addr + HES(0x004)) = MEM16(VR[vt][06]);
        *(pi16)(DMEM + addr + HES(0x006)) = MEM16(VR[vt][07]);
        break;
    case 0x6/2:
        *(pi16)(DMEM + addr + HES(0x000)) = MEM16(VR[vt][05]);
        *(pi16)(DMEM + addr + HES(0x002)) = MEM16(VR[vt][06]);
        *(pi16)(DMEM + addr + HES(0x004)) = MEM16(VR[vt][07]);
        break;
    case 0x4/2:
        *(pi16)(DMEM + addr + HES(0x000)) = MEM16(VR[vt][06]);
        *(pi16)(DMEM + addr + HES(0x002)) = MEM16(VR[vt][07]);
        break;
    case 0x2/2:
        *(pi16)(DMEM + addr + HES(0x000)) = MEM16(VR[vt][07]);
        break;
    case 0x0/2:
        break;
    }
...
void LTV(unsigned vt, unsigned element, signed offset, unsigned base)
{
    ...
    for (i = 0; i < 8; i++) /* SGI screwed LTV up on N64.  See STV instead. */
        VR[vt + i][(i - e/2) & 07] = MEM16(*(pi16)(DMEM + addr + HES(2*i)));
    return;
}
...
void STV(unsigned vt, unsigned element, signed offset, unsigned base)
{
    ...
    for (i = 0; i < 8; i++)
        *(pi16)(DMEM + addr + HES(2*i)) = MEM16(VR[vt + (e/2 + i)%8][i]);
    return;
}
...
PROFILE_MODE int SPECIAL(u32 inst, u32 PC)
{
    ...
    case 010: /* JR */
        ...
        return 1;
    case 015: /* BREAK */
        *CR[0x4] |= REGVAL(SP_STATUS_BROKE | SP_STATUS_HALT);
        if (REGVAL(*CR[0x4]) & SP_STATUS_INTR_BREAK) {
...
NOINLINE void run_task(void)
{
    ...
    PC = FIT_IMEM(REGVAL(GET_RCP_REG(SP_PC_REG)));
    for (;;) {
        inst_word = INSTWORD(*(pi32)(IMEM + FIT_IMEM(PC)));

        if (callback_inst_debug != NULL) {
            callback_inst_debug(FIT_IMEM(PC), inst_word);
        }
...
EX:
...
#if (0 != 0)
        if (GET_RCP_REG(SP_STATUS_REG) & REGVAL(SP_STATUS_HALT))
        ...
        if (stage == 2) { /* branch phase of scheduler */
            stage = 0*stage;
            PC = FIT_IMEM(temp_PC);
            GET_RCP_REG(SP_PC_REG) = REGVAL(temp_PC);
        } else {
            stage = 2*stage; /* next IW in branch delay slot? */
            PC = FIT_IMEM(PC + 0x004);
            GET_RCP_REG(SP_PC_REG) = REGVAL(0x04001000 + PC);
        }
#else
        continue;
set_branch_delay:
...
RSP_halted_CPU_exit_point:
    GET_RCP_REG(SP_PC_REG) = REGVAL(0x04001000 | FIT_IMEM(PC));
    return;
}

NOINLINE void single_step(void)
{
    register u32 PC;
