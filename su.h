...
extern inst_debug_func callback_inst_debug;
...
#define SR_B(scalar, i)         *((unsigned char *)&(SR[scalar]) + PTR_BYTE(i))
 ...
 */
#define VR_B(vt, element)       *((unsigned char *)&(VR[vt][0]) + PTR_BIMI(element))
 ...
 */
#define VR_A(vt, e)             *((unsigned char *)&(VR[vt][0]) + e + PTR_BIMI(0))
#define VR_U(vt, e)             *((unsigned char *)&(VR[vt][0]) + e - PTR_BIMI(0))
...
NOINLINE extern void single_step(void);
